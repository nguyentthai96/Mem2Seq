...
def read_langs(file_name, entity, max_line=None):
                ...
                if '\t' in line:
                    ...
                    for key in r.split(' '):
                        if ENTPTR:
                            ...
                            index = [loc for loc, val in enumerate(contex_arr) if
                                     (val[0] == key)]  # check index of word robot exist in question of people
                            if (index):
                                index = max(index)
                                gate.append(1)
                                cnt_ptr += 1
                    ...
                    data.append(
                        [contex_arr_temp, r, r_index, gate, list(conversation_arr), ent, dialog_counter, kb_arr])
    ...
    print('NTT Sample: utils babi mem2seq 261', data[1][0], data[1][1], data[1][2], data[1][3])
...
def generate_memory(sent, speaker, time):
    ...
    if speaker == "$u" or speaker == "$s":  # split word in statement user
    ...
    return sent_new  # array split word in with same t<number> [word, speaker, t<number>]
